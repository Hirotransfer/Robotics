...
/*-------------------------------------------------------------------*/
/*!
    bhv_set_play 是非 playon 模式下的模块。该文件在 src 目录下，调用接口在 sample_player.cpp
文件中：actionImpl()函数中实现。

    setplay 有如下几种模式，均在 src 目录下：
    bhv_goalie_free_kick //门将的任意球（自由球）
    bhv_set_play_free_kick //球员的任意球
    bhv_set_play_goal_kick //门球模式
    bhv_set_play_kick_off //开球模式
    bhv_set_play_kick_in //边界球模式
    bhv_set_play_indirect_free_kick //直接任意球模式
    
    各种非 playon 模式下均有各自的阵型文件
    
    对于发球方，主要逻辑是确定踢球者，站位，发球（一般用动作链模式）。
对于防守方，主要逻辑是根据阵型点确定位置，观察阵型点附近有无对方球
员，若有，适时上去压对方。
...
Bhv_SetPlay::execute( PlayerAgent * agent )
{
    ...
    // 球员逻辑
    switch ( wm.gameMode().type() ) {   // 判断比赛模式
    case GameMode::KickOff_:
        ...
        break;
    case GameMode::KickIn_:   // 边界球
    case GameMode::CornerKick_: // 角球
        if ( wm.gameMode().side() == wm.ourSide() )
        {
    ...
    case GameMode::GoalKick_: // 门球
        ...
        break;
    case GameMode::BackPass_: // 门将禁区接本方球员回传球
    case GameMode::IndFreeKick_:  // 直接任意球
        return Bhv_SetPlayIndirectFreeKick().execute( agent );
        break;
    case GameMode::FoulCharge_: // 犯规
